                            ...
                            <div class="form-group">
                                ...
                                <select name="service_category_id" id="service_category_id" class="form-control select2">
                                    ...
                                    @foreach($service_categories as $cat)
                                        <option value="{{$cat->id}}" {{$cat->id == $blog->service_category_id ? 'selected': ''}}>{{$cat->category}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="short_description">Post Short Description</label>
                                <textarea type="text" class="form-control" name="short_description" id="short_dssescription" placeholder="Enter Post Short Description">{{ $blog->short_description }}</textarea>
                            </div>
                            <div class="form-group">
                                <label for="description">Post Description</label>
                                <textarea type="text" class="form-control textarea" name="description" id="description" placeholder="Enter Post Title">{!! $blog->description !!}</textarea>
                            </div>
                            <div class="text-left">
                                <img class="profile-user-img img-fluid"
                                     src="{{asset('uploads/post/'.$blog->image)}}"
